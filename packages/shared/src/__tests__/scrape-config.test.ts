...
const VALID_STEPS: Array<{ op: string; step: Step }> = [
  { op: 'goto', step: { op: 'goto', url: 'https://example.com', waitUntil: 'networkidle' } },
  { op: 'waitFor', step: { op: 'waitFor', selector: '#ready', timeoutMs: 500, state: 'attached' } },
  { op: 'click', step: { op: 'click', selector: 'a.next', opens: 'newTab', optional: true } },
  ...
  { op: 'capture', step: { op: 'capture', as: ['PNG', 'PDF'], name: 'r-{{index}}', fullPage: false } },
  {
    op: 'forEach',
  ...
  {
    op: 'openLink',
    step: { op: 'openLink', selector: 'a', attribute: 'data-href', steps: [{ op: 'goBack' }] },
  },
  {
    op: 'paginate',
    step: { op: 'paginate', nextSelector: 'a.next', maxPages: 3, steps: [{ op: 'goBack' }] },
  },
...
describe('validateScrapeConfig v2 accept', () => {
  it.each(VALID_STEPS)('accepts a valid $op step', ({ step }) => {
    expect(validateScrapeConfig(withSteps([step]))).toEqual({ version: 2, steps: [step] });
  });
...
describe('validateScrapeConfig v2 reject', () => {
  it.each([
    ...
    { desc: 'click with an unknown opens', step: { op: 'click', selector: 'a', opens: 'window' } },
    { desc: 'fill with both value and valueFrom', step: { op: 'fill', selector: '#p', value: 'a', valueFrom: 'b' } },
    { desc: 'fill with neither value nor valueFrom', step: { op: 'fill', selector: '#p' } },
    { desc: 'select without a value', step: { op: 'select', selector: 'select' } },
    { desc: 'press without a key', step: { op: 'press' } },
    { desc: 'scroll to element without a selector', step: { op: 'scroll', to: 'element' } },
    { desc: 'extract without fields', step: { op: 'extract', name: 'rows', fields: [] } },
    { desc: 'extract with a bad emit', step: { op: 'extract', name: 'r', fields: [{ name: 'a', selector: 'b' }], emit: ['PNG'] } },
    { desc: 'capture with a non-capture type', step: { op: 'capture', as: ['WEBM'], name: 'x' } },
    { desc: 'capture without a name', step: { op: 'capture', as: ['PNG'] } },
    { desc: 'forEach without nested steps', step: { op: 'forEach', rowSelector: 'tr', steps: [] } },
    { desc: 'openLink without nested steps', step: { op: 'openLink', selector: 'a', steps: [] } },
    { desc: 'paginate without maxPages', step: { op: 'paginate', nextSelector: 'a', steps: [{ op: 'goBack' }] } },
    { desc: 'a nested step that is malformed', step: { op: 'forEach', rowSelector: 'tr', steps: [{ op: 'nope' }] } },
  ])('rejects $desc', ({ step }) => {
  ...
  it.each([
    ...
    { desc: 'an unknown auth mode', input: { version: 2, steps: [{ op: 'goBack' }], auth: { mode: 'oauth' } } },
  ])('rejects $desc', ({ input }) => {
...
describe('limits', () => {
  ...
  it('rejects a non-positive limit', () => {
    expect(() =>
      validateScrapeConfig({ version: 2, steps: [{ op: 'goBack' }], limits: { maxPages: 0 } }),
    ).toThrow();
  });
...
describe('isV1Config', () => {
  it.each([
    { desc: 'a config with no version', input: { fields: [], artifacts: [] }, expected: true },

...
function ChromeProfileFields({
  ...
  return (
    <div className="field-row">
      <label htmlFor="auth-userdatadir">Chrome user data directory</label>
      <input
        id="auth-userdatadir"
        value={auth.userDataDir}
        onChange={(e) => onChange({ ...auth, userDataDir: e.target.value })}
        placeholder="/Users/you/Library/Application Support/Google/Chrome"
      />
      <label htmlFor="auth-profiledirectory">Profile directory</label>
      <input
        id="auth-profiledirectory"
        value={auth.profileDirectory ?? ''}
        onChange={(e) => onChange(withOptional(auth, 'profileDirectory', e.target.value))}
        placeholder="Default"
      />
    </div>
  );
...
export function AuthConfigEditor({
  ...
  return (
    <div>
      ...
      {auth.mode === 'chromeProfile' ? (
        <ChromeProfileFields auth={auth} onChange={onChange} />
      ) : null}
